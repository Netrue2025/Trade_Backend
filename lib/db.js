...
function getChangedAppStateFields(snapshot) {
...
function createAppStateSaveController({
  ...
  async function runMongoSave(snapshot, requestCount, contexts) {
    ...
    const observed = await withSlowOperationWarning(
      saveMongo(collection, snapshot, { logger, saveId, contexts }),
      "app_state save",
      getSlowWarningMs(),
      logger
    );
    ...
    logger.log(`[persistence] app_state save completed in ${observed.durationMs}ms; coalesced=${coalescedRequests}; dirtyPending=${dirty ? "yes" : "no"}`);
  }
  ...
  async function runWorker() {
    ...
    try {
      while (dirty) {
        ...
        try {
          await runMongoSave(snapshot, requestCount, contexts);
        } catch (error) {
  ...
  function requestSave(db, context = null) {
    ...
    dirty = true;
    pendingRequests += 1;
    if (context && typeof context === "object") pendingContexts.push({ ...context });
    if (!workerPromise) {
...
async function updateMongoAppState(collection, snapshot, { logger = console, saveId = 0 } = {}) {
  const snapshotToSave = normalizeDb(snapshot);
  delete snapshotToSave._id;
  const changedFields = getChangedAppStateFields(snapshotToSave);
...
async function saveMongoSnapshot(collection, snapshot, { logger = console, backupSnapshot = backupMongoSnapshot, saveId = 0 } = {}) {
  const backupDue = isMongoBackupDue();
  const saveStartedAt = Date.now();
  logPersistencePhase(logger, `save started backupDue=${backupDue ? "yes" : "no"}`);

  if (!backupDue) {
    await updateMongoAppState(collection, snapshot, { logger, saveId });
    logPersistencePhase(logger, `save completed duration=${Date.now() - saveStartedAt}ms`);
    return;
  }

  const currentSnapshot = await readCurrentMongoSnapshotForSave(collection, { logger });
  const snapshotToSave = currentSnapshot
    ? normalizeDb(mergeMongoSnapshots(snapshot, currentSnapshot))
    : snapshot;
  await updateMongoAppState(collection, snapshotToSave, { logger, saveId });

  if (currentSnapshot) {
    markMongoBackupAttempt();
    logPersistencePhase(logger, "backup scheduled");
    void backupSnapshot(currentSnapshot, { logger }).catch((error) => {
...
function saveDb(db, options = {}) {
  ...
  return appStateSaveController.requestSave(authoritativeDb, options.context);
}
