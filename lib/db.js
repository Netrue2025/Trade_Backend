...
function getMongoBackupIntervalMs() {
  ...
  return configured ? toBoundedNumber(configured, 300000, 0, 24 * 60 * 60 * 1000) : 300000;
}
...
function getMongoBackupLimit() {
  const configured = Number(getEnvValue("MONGODB_APP_STATE_BACKUP_LIMIT") || 50);
  return Number.isFinite(configured) ? Math.max(5, Math.min(Math.floor(configured), 500)) : 50;
}
