...
function getMongoBackupLimit() {
  const configured = Number(getEnvValue("MONGODB_APP_STATE_BACKUP_LIMIT") || 5);
  return Number.isFinite(configured) ? Math.max(5, Math.min(Math.floor(configured), 500)) : 5;
}
...
async function backupMongoSnapshot(snapshot, { logger = console } = {}) {
  ...
  const excess = await backupCollection
    .find({ appStateId })
    .sort({ createdAt: -1 })
    .skip(backupLimit)
    .project({ _id: 1 })
    .toArray();
  if (excess.length) {
    await backupCollection.deleteMany({ _id: { $in: excess.map((item) => item._id) } });
  }
