...
async function backupMongoSnapshot(snapshot) {
  if (!snapshot) {
    return;
  }
  const db = await getMongoDb();
...
async function saveMongoSnapshot(collection, snapshot) {
  ...
  if (shouldUseFullAppStateOperations()) {
    ...
    if (currentSnapshot && shouldBackupMongoSnapshot()) {
      void backupMongoSnapshot(currentSnapshot).catch((error) => {
