...
function createAppStateSaveController({
  ...
  async function runMongoSave(snapshot, requestCount) {
    ...
    const observed = await withSlowOperationWarning(
      saveMongo(collection, snapshot, { logger }),
      "app_state save",
      getSlowWarningMs(),
      logger
    );
  ...
  async function runWorker() {
    ...
    try {
      while (dirty) {
        ...
        try {
          await runMongoSave(snapshot, requestCount);
        } catch (error) {
...
async function updateMongoAppState(collection, snapshot, { logger = console } = {}) {
  const snapshotToSave = normalizeDb(snapshot);
  delete snapshotToSave._id;
  const startedAt = Date.now();
  logPersistencePhase(logger, "update started");
  try {
    if (shouldUseFullAppStateOperations()) {
      ...
      logPersistencePhase(logger, `update completed duration=${Date.now() - startedAt}ms`);
      return;
    }
    ...
    throw wrapMongoAppStateError("update", error);
  }
}
...
async function saveMongoSnapshot(collection, snapshot, { logger = console, backupSnapshot = backupMongoSnapshot } = {}) {
  const backupDue = isMongoBackupDue();
  const saveStartedAt = Date.now();
  logPersistencePhase(logger, `save started backupDue=${backupDue ? "yes" : "no"}`);

  if (!backupDue) {
    await updateMongoAppState(collection, snapshot, { logger });
  ...
  const snapshotToSave = currentSnapshot
    ...
    : snapshot;
  await updateMongoAppState(collection, snapshotToSave, { logger });
