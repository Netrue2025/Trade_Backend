...
function isSymbolWhitelistError(message) {
  return String(message || "").toLowerCase().includes("symbol is not whitelisted");
}

function buildActionableErrorMessage(payload, fallback, params = {}) {
  const message = normalizeErrorMessage(payload, fallback);
  if (!isSymbolWhitelistError(message)) {
    return message;
  }

  const symbol = String(params.symbol || "").trim().toUpperCase();
  return [
    `Bybit rejected ${symbol || "this symbol"} because the connected API key or sub-account is restricted by a symbol whitelist.`,
    "Create or edit the Bybit API key as a System-generated Read-Write key with Spot Trade permission for the Unified Trading Account, and do not limit it to selected symbols.",
    symbol ? `Alternatively, add ${symbol} to the allowed symbols for that Bybit API key/sub-account.` : "",
  ].filter(Boolean).join(" ");
}
...
function formatHttpFailure(url, response, payload, signed = false, params = {}) {
  const fallback = signed
    ? `Signed Bybit request failed with HTTP ${response.status}.`
    : `Unable to reach Bybit. HTTP ${response.status}.`;
  const message = buildActionableErrorMessage(payload, fallback, params);
...
async function publicRequest(path, params = {}, testnet = false) {
  ...
  for (const baseUrl of getBaseUrls(testnet)) {
    ...
    if (!response.ok) {
      lastHttpFailure = new Error(formatHttpFailure(url, response, payload, false, params));
...
async function signedRequest(account, path, method = "GET", params = {}, allowRetry = true) {
  ...
  for (const baseUrl of getBaseUrls(account.testnet)) {
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) {
      lastHttpFailure = new Error(formatHttpFailure(url, response, payload, true, params));
      if (isRetryableHttpStatus(response.status)) {
        continue;
      }
      throw lastHttpFailure;
    ...
    if (payload.retCode !== 0) {
      ...
      throw new Error(buildActionableErrorMessage(payload, `Signed Bybit request failed with HTTP ${response.status}.`, params));
    }
...
function normalizeWalletResponse(result) {
  ...
  return {
    ...
    totalEquity: String(account?.totalEquity || ""),
    totalWalletBalance: String(account?.totalWalletBalance || ""),
    totalAvailableBalance: String(account?.totalAvailableBalance || ""),
    totalPerpUPL: String(account?.totalPerpUPL || ""),
    accountIMRate: String(account?.accountIMRate || ""),
    accountMMRate: String(account?.accountMMRate || ""),
...
async function getBookTicker(symbol, testnet) {
  const normalizedSymbol = String(symbol || "").trim().toUpperCase();
  const result = await withPublicCache(
    getPublicCacheKey("bookTicker", [testnet ? "testnet" : "mainnet", normalizedSymbol]),
    PUBLIC_CACHE_TTLS_MS.bookTicker,
    () => publicRequest("/v5/market/orderbook", { category: "spot", symbol: normalizedSymbol, limit: 1 }, testnet)
  );
  const bestBid = Array.isArray(result.b?.[0]) ? result.b[0] : [];
  const bestAsk = Array.isArray(result.a?.[0]) ? result.a[0] : [];
  return {
    symbol: normalizedSymbol,
    bidPrice: String(bestBid[0] || "0"),
    askPrice: String(bestAsk[0] || "0"),
  };
}
...
async function placeSpotOrder(account, orderInput) {
  const isPostOnlyLimit = orderInput.type === "LIMIT" && String(orderInput.timeInForce || "").toUpperCase() === "POST_ONLY";
  const params = {
    category: "spot",
    ...
    timeInForce: orderInput.type === "LIMIT" ? (isPostOnlyLimit ? "PostOnly" : orderInput.timeInForce || "GTC") : "IOC",
    orderFilter: "Order",
  };
