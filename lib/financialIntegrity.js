...
class FinancialIntegrityState {
  ...
  freeze(reason = "required_financial_persistence_failed") {
    if (!this.frozen) {
      this.frozen = true;
      this.reason = String(reason || "required_financial_persistence_failed");
      this.frozenAt = new Date().toISOString();
      console.error("FINANCIAL_PERSISTENCE_FROZEN", { reason: this.reason });
    }
  }

  getStatus() {
    return { writable: this.authoritativeStateRegistered && !this.frozen, persistenceFrozen: this.frozen, frozenAt: this.frozenAt };
  }
}
