...
function getBackupSortValue(backup = {}) {
  return normalizeCreatedAt(backup.createdAt);
}
...
function classifyBackup(backup = {}, appStateId = DEFAULT_APP_STATE_ID) {
  ...
  if (!normalizeCreatedAt(backup.createdAt)) {
    return "malformed";
  }
  return "valid";
}
...
function planAppStateBackupCleanup(backups = [], { keep = DEFAULT_KEEP_COUNT, appStateId = DEFAULT_APP_STATE_ID } = {}) {
  ...
  return {
    ...
    newestProtected: eligibleForDeletion.every((backup) => !keepIds.has(getBackupIdText(backup))),
  };
}
