...
async function getExchangeInfo(symbol, testnet) {
  ...
  return {
    symbols: symbols.map((item) => ({
      ...
      quotePrecision: item.quotePrecision ?? item.quoteAssetPrecision ?? "",
...
async function getBookTicker(symbol, testnet) {
  ...
  const result = await withPublicCache(
    ...
    () => publicRequest("/api/v3/ticker/bookTicker", { symbol: normalizedSymbol }, testnet)
  );
  return {
    symbol: normalizedSymbol,
    bidPrice: String(result.bidPrice || "0"),
    askPrice: String(result.askPrice || "0"),
  };
}
...
async function placeSpotOrder(account, orderInput) {
  ...
  const params = {
    ...
    type: isPostOnlyLimit ? "LIMIT_MAKER" : orderInput.type,
    quantity:
    ...
    timeInForce: orderInput.type === "LIMIT" && !isPostOnlyLimit ? orderInput.timeInForce || "GTC" : undefined,
